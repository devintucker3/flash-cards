...
function Cards({singleDeck, frontView, setFrontView, index, setIndex}) {
    ...
    const nextHandler = () => {
        if (index++ === singleDeck.cards.length) {
    ...
    return (
        <div className="card">
            <div className="card-body">
                <h5 className="card-title">Card {index++} of {singleDeck.cards.length}</h5>
                <p className="card-text">
                    {Object.keys(singleDeck).length !== 0 ? (
                        frontView ? singleDeck.cards[index].front : singleDeck.cards.back
                    ) : null}
                </p>
